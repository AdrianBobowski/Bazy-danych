...
SELECT * FROM księgowosc.pracownicy WHERE księgowosc.pracownicy.nazwisko LIKE '%n%' AND księgowosc.pracownicy.imie LIKE '%a';
...
SELECT pracownicy.imie, pracownicy.nazwisko, księgowosc.pensja.kwota AS pensja, księgowosc.premia.kwota AS premia
FROM księgowosc.pracownicy
INNER JOIN księgowosc.wynagrodzenia
ON księgowosc.wynagrodzenia.id_pracownika = księgowosc.pracownicy.id_pracownika
INNER JOIN księgowosc.pensja
...
INNER JOIN księgowosc.premia
ON księgowosc.premia.id_premii = księgowosc.wynagrodzenia.id_premii
ORDER BY księgowosc.pensja.kwota DESC, księgowosc.premia.kwota DESC;

---k)

SELECT pensja.stanowisko, COUNT(wynagrodzenia.id_pracownika) AS liczba_pracownikow
FROM księgowosc.pensja
LEFT JOIN księgowosc.wynagrodzenia ON pensja.id_pensji = wynagrodzenia.id_pensji
GROUP BY pensja.stanowisko;

---l)

SELECT AVG(pensja.kwota) AS srednia_placa, MIN(pensja.kwota) AS minimalna_placa, MAX(pensja.kwota) AS maksymalna_placa
FROM księgowosc.pensja 
WHERE pensja.stanowisko = 'IT Project Manager';

---m)

SELECT SUM(pensja.kwota) + SUM(premia.kwota) AS suma_wynagrodzen
FROM księgowosc.pensja
JOIN księgowosc.wynagrodzenia 
ON pensja.id_pensji = wynagrodzenia.id_pensji
JOIN księgowosc.premia 
ON premia.id_premii = wynagrodzenia.id_premii;

---f)

SELECT pensja.stanowisko, SUM(pensja.kwota) + SUM(premia.kwota) AS suma_wynagrodzen
FROM księgowosc.pensja
JOIN księgowosc.wynagrodzenia 
...
SELECT pensja.stanowisko, COUNT(premia.kwota) AS suma_premii
FROM księgowosc.pensja 
JOIN księgowosc.wynagrodzenia 
ON wynagrodzenia.id_pensji = pensja.id_pensji 
JOIN księgowosc.premia
